...
function prepareSleepData() {
    ...
    for (const date in groupedByDate) {
        ...
        groupedByDate[date].forEach(data => {
            ...
            if (data.sleepDuration !== undefined && data.sleepDuration !== null) {
                // Вариант 1: строка формата "ЧЧ:ММ"
                if (typeof data.sleepDuration === 'string') {
                    const parts = data.sleepDuration.split(':');
                    sleepHours = parseInt(parts[0]) || 0;
                    sleepMinutes = parseInt(parts[1]) || 0;
                }
                // Вариант 2: число (часы в десятичном формате)
                else if (typeof data.sleepDuration === 'number') {
                    sleepHours = Math.floor(data.sleepDuration);
                    sleepMinutes = Math.round((data.sleepDuration - sleepHours) * 60);
                }
                // Вариант 3: объект с часами и минутами
                else if (typeof data.sleepDuration === 'object') {
                    sleepHours = data.sleepDuration.hours || 0;
                    sleepMinutes = data.sleepDuration.minutes || 0;
                }

                totalSleepHours += sleepHours + (sleepMinutes / 60);
                sleepCount++;
            }
...
function prepareEnergyData() {
    ...
    for (const date in groupedByDate) {
        ...
        groupedByDate[date].forEach(data => {
            // Безопасная обработка уровня энергии
            if (data.energyLevel !== undefined && data.energyLevel !== null) {
                let level = data.energyLevel;
...
function calculateOneRepMax(weight, reps) {
    ...
    try {
        ...
        console.error('Ошибка расчета 1ПМ:', e);
        return 0;
    }
}
